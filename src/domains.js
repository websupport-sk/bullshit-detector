...
// RegEx (,).*
async function getDomainsFromServer() {
    let domainScores = {}

    try {
        const domainsFile = (await fetch('http://127.0.0.1:8887/zoznam.txt')).text();
        const text = await domainsFile;
...
if (incompleteData || obsoleteData) {
    domainScores = getDomainsFromServer();
}
export const domains = domainScores;
